...
You can give more informative message by putting a hint message in the
...
<p><Alert severity="warning">
Necode uses compiler transformations to detect <code>SHOW_TEST</code> as well as to modify the call
signature of <code>check</code> and other testing DSL builtins. Something like this:
...
<Alert severity="warning">
    ...
    While you can circumvent this by repeatedly calling <code>wait</code> multiple times, it is not recommended.
...
<Alert severity="error">
    SyntaxTransformError: <code>check</code> cannot be used in a synchronous context.
    Add the <code>async</code> keyword to the surrounding function.
</Alert>